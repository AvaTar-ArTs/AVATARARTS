#!/usr/bin/env python3
"""
Generate HTML Documentation System from Directory Analysis
"""
...
def convert_size_to_bytes(size_str):
    ...
    if 'kb' in size_str:
        ...
        return float(size_str.replace(' bytes', '').replace('byte', ''))
    else:
        # Assume it's already in bytes if no unit specified
        try:
...
<body>
    ...
    <script>
        ...
        const fileData = ''' + json.dumps(generate_js_data(load_analysis_data(Path.home() / 'targeted_deep_dive_analysis.csv')), indent=2) + ''';
        
        // Rest of the JavaScript code will be added here
    </script>
...
def create_final_script():
    """Create the final JavaScript file with embedded data"""
    js_content = '''// final_script.js - Final JavaScript functionality for AVATARARTS Directory Documentation System
...
def main():
    """Main function to generate the documentation system"""
    print("Generating HTML Documentation System from directory analysis...")
    ...
    js_content = create_final_script()
    
    # Write the JavaScript file
    js_path = Path(__file__).parent / 'final_script.js'
    with open(js_path, 'w', encoding='utf-8') as f:
        f.write(js_content)
    
    print(f"Documentation system generated successfully!")
