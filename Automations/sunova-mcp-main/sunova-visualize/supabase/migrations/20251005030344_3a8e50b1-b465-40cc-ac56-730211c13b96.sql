...
-- Create admin settings table if it doesn't exist (for email templates)
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_tables WHERE tablename = 'admin_settings' AND schemaname = 'public') THEN
    -- Table doesn't exist yet, no need to check for specific settings
    NULL;
  ELSE
    -- Table exists, insert default email templates if they don't exist
...
-- Policy: Admins can update beta applications (drop if exists, then create)
DO $$
BEGIN
